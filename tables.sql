...
\copy Reaction(idPubli, idUser, type) FROM 'CSV/reaction.csv' WITH (FORMAT csv, HEADER true);
...
\copy InteresseEvent(userId, eventId) FROM 'CSV/interesse_event.csv' WITH (FORMAT csv, HEADER true);
\copy EventParticulier(id, auteur, nomEvent, dateEvent, lieuEvent, nbPlaceDispo, nbPlaceReserve, organisateur, liens_web) FROM 'CSV/events.csv' WITH (FORMAT csv, HEADER true);
\copy Publication(id, auteur, discussionId, titre, contenu, sujetId, datePublication, parentId) FROM 'CSV/publications.csv' WITH (FORMAT csv, HEADER true);
